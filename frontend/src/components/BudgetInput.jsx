export default function BudgetInput({ value, onChange, error }) {
  return (
    <div>
      ...
      <input id="budget" name="budget" className="form-input" type="number" inputMode="decimal" min="0" step="0.01" required value={value} onChange={event => onChange(event.target.value)} aria-invalid={Boolean(error)} aria-describedby={error ? 'budget-help budget-error' : 'budget-help'} />
