...
export default function MapView({ activities = DEFAULT_LOCATIONS, selectedId, onSelect }) {
  ...
  return (
    <section className="route-map" aria-label="Itinerary map">
      ...
      <div className="route-map__canvas">
        <svg viewBox="0 0 800 460" role="img" aria-label={`Map with ${points.length} itinerary stops`}>
          <defs>
          ...
          <rect width="800" height="460" fill="url(#map-grid)" />
          ...
          {points.map((point) => {
            ...
            return (
              <g
                ...
                onKeyDown={(event) => {
                  if (event.key === "Enter" || event.key === " ") selectPoint(point);
                }}
                role="button"
                tabIndex="0"
                aria-label={`Stop ${point.index + 1}: ${point.name}`}
              >
                <circle r={isActive ? 24 : 20} filter="url(#marker-shadow)" />
        ...
        {active && (
          <div className="route-map__popover">
      ...
      <ol className="route-map__legend">
        {points.map((point) => (
          <li key={point.id || point.index}>
            <button type="button" onClick={() => selectPoint(point)}>
